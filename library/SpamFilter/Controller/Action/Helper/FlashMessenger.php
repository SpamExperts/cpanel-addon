 ...
 */
class SpamFilter_Controller_Action_Helper_FlashMessenger extends Zend_Controller_Action_Helper_Abstract implements IteratorAggregate, Countable
{
    /**
    ...
    {
        if (!self::$_session instanceof SpamFilter_Session_Namespace) {
            self::$_session = new SpamFilter_Session_Namespace($this->getName());
    ...
    public function addMessage($message)
    {
        ...
        // Do not register duplicate messages
    ...
    /**
     * messageExists() - checks if message exists
     * @param $message array()
     * @return boolean
     */
    ...
    {
        if ($this->hasCurrentMessages()) {
            foreach (self::$_session->{$this->_namespace} as $queued) {
                if ($queued['message'] === $message['message'] &&
                    $queued['status']  === $message['status']
                ) {
                    return true;
                }
            }
